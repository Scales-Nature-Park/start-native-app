...
const DataInput = ({ params }) => {
    ...
    const displayField = (field, fields) => {
        ...
        if (field.dropDown) {
            let dropVals = [];
            let initId = (params) ? -1 : 0;
            let initValue = params?.inputFields?.filter((element) => element.name.toLowerCase() == field.name.toLowerCase())[0];
